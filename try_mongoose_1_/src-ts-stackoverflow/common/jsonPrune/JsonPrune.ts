...
import {when, whenEqual} from "../Commons";
...
function indent(level:number,spacePerIndent:number=2):string {
    let str = "";
    let tab = space(spacePerIndent);
...
function jsonDeepStringify__(obj:any,indentLevel:number,spacePerIndent:number=2,avoidKeys:string[]){
    try{
        //console.log("{");
...
function jsonDeepStringifyArray__(arr:any[],indentLevel:number,spacePerIndent:number=2,avoidKeys:string[]){
    try{
        // console.log("[");
        for(let item of arr){
            if(item==null){
...
export namespace JSONx{
    export function logJsonDeep(value:any,spacePerIndent:number=2,avoidKeys:string[]=null){
        if(value==null){
            console.log("null");
        }else if(Array.isArray(value)){
            console.log("[");
