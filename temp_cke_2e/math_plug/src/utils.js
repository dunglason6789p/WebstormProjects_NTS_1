	...
	if ( engine === 'mathjax' && typeof MathJax !== 'undefined' ) {
		if ( isMathJaxVersion3( MathJax.version ) ) {
			...
			selectRenderMode( element, preview, previewUid, el => {
				...
				// Fixme(cont)...so as a work-around I have to call like this:
...
/**Assign `text equation` to `innerHTML` of target `element`(maybe a blank div), THEN call MathJax to render on that target `element`*/
